...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person, data in people.items():
        ...
        if not mother and not father:
            ...
            probability *= (
                inheritance[mother] * inheritance[father]
            ) if person in two_genes else ((
                inheritance[mother] * (1 - inheritance[father])
            ) + (inheritance[father] * (1 - inheritance[mother]))
...
def update(probabilities, one_gene, two_genes, have_trait, p):
    ...
    for person in probabilities:
        genes = 1 if person in one_gene else 2 if (
            person in two_genes
        ) else 0
